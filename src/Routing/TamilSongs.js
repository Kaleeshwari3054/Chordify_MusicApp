...
import "../components/Bolt.css";  // Custom styles for TamilSongs component
...
function TamilSongs() {
  ...
  useEffect(() => {
    fetch("/Boltse.json")
      .then((res) => res.json())
      .then((data) => {
        // Access the songs array inside the object
        const tamilSongs = data.songs.filter((song) => song.language === "Tamil");
        setSongs(tamilSongs);
      })
      .catch((error) => {
